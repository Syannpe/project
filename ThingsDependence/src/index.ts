...
export default class Index {
    private elementToTreeNode: Map<string, TreeNode> = new Map();

    #dfs(node: TreeNode, level: number, cb: (node: TreeNode, level: number) => void) {
        // 递归终止条件
        if (!node) {
            return;
        }
    ...
    getAllDependents(roots: TreeNode[], cb: (node: TreeNode, level: number) => void) {
