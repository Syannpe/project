...
export default class Linear extends Command {
    ...
    public render(): Object {
        ...
        let earlyEnd = tasks.reduce((earlyEnd, task) =>
            earlyEnd > ChartTable.getInstance().calcRelativeTime(task.earlyEnd, task.earlyStart) ?
                earlyEnd :
                ChartTable.getInstance().calcRelativeTime(task.earlyEnd, task.earlyStart),
            ChartTable.getInstance().calcRelativeTime(tasks[0].earlyEnd, tasks[0].earlyStart));

        let lateEnd = tasks.reduce((lateEnd, task) =>
                lateEnd > ChartTable.getInstance().calcRelativeTime(task.lateEnd, task.earlyStart) ?
                    lateEnd :
                    ChartTable.getInstance().calcRelativeTime(task.lateEnd, task.earlyStart),
            ChartTable.getInstance().calcRelativeTime(tasks[0].lateEnd, tasks[0].earlyStart));

        const row = html.drawRow(tasks[0].id, tasks[0].name,
            earlyStart, lateStart,
            earlyEnd, lateEnd,
            tasks[0].color, tasks[0].equation);

        tasks.forEach(task => {
            const options = super.render();

            html.drawDate(row, task.earlyStart, task.lateStart,
                ChartTable.getInstance().calcRelativeTime(task.earlyEnd, task.earlyStart),
                ChartTable.getInstance().calcRelativeTime(task.lateEnd, task.earlyStart),
