...
class HTMLInterface {
    ...
    public rowDate: TableCellStatus[][] = [];

    public drawRow(id: string, name: string, earlyStart: Date, lateStart: Date, earlyEnd: Date, lateEnd: Date, color: string, equation?: (date: Date, dayNumber: number) => boolean) {
    ...
    // 绘制“时间轴”用的重载方法
    public drawDate(rowNum: number, earlyStart: Date, lateStart: Date, earlyEnd: Date, lateEnd: Date, proportion: number | number[], options?: DrawOptions): void {
        // 计算当前时间和初始时间差了多少天
        let toRelativeDate = (time: Date) => {
            return Math.floor((time.getTime() - this.startTime.getTime()) / 1000 / 60 / 60 / 24);
        };
        let toAbsoluteDate = (days: number) => {
    ...
    public drawCache(rowNum: number, proportion: number | number[] = 1, options?: DrawOptions) {
    ...
    */
    private remainingDaysByMonthList(start: Date | string, end: Date | string) {
        const s = start instanceof Date ? start : new Date(start);
        const e = end instanceof Date ? end : new Date(end);
        if (s > e) throw new Error("开始日期必须早于或等于结束日期");
        ...
        const result: Array<{ year: number; month: number; daysRemaining: number[] }> = [];
    ...
    public static instance: HTMLInterface;

    private constructor() {
    }

    /** 获取单例 */
    public static getInstance(): HTMLInterface {
